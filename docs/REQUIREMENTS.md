...
### 3.13 飲酒安全サポート（LINE、新規提案・未実装）
飲酒運転・事故のリスクを減らすため、飲酒開始時に帰宅手段をその場で確保できるようにする機能。**2026-07-06に要件を確定。実装は次フェーズ以降（本セッションでは着手しない）。**

**フロー**
1. LINEのリッチメニューから「飲み始めた」を選択（起動トリガー）。
2. LINEのクイックリプライで杯数・お酒の種類を選択（既存`lib/alcohol-calc.ts`の`DRINK_PRESETS`を流用: ビール/ハイボール/チューハイ/日本酒/ワイン等）。
3. LINEのネイティブ位置情報共有機能でGPS座標を送信してもらう。
4. サーバー側で以下を行い、その場で1通にまとめてLINE返信:
   - 純アルコール量→分解時間→酔いが覚める予測時刻を計算（既存`calculateDecompositionTime`を再利用）。
   - GPS座標に基づき該当エリアの代行運転・タクシーの電話番号を検索。**データソースは手動登録を優先し、該当エリアが未登録の場合のみGoogle Places API等でフォールバック**（代行運転は公的オープンデータが存在しないため）。
   - 電話番号は`tel:`リンクのボタンとして送信し、タップ→電話アプリのワンタップ発信画面まで到達できるようにする（LINE Botからの自動発信は仕組み上不可能なため、ここが「ワンタップ」の実現可能な上限）。
5. **最初の返信時点で代行/タクシー番号も同時に提示する**（「酔いが覚めてから番号を送る」のではなく、飲み始めた直後にすぐ帰宅手段がわかる状態にする）。
6. 予測時刻が近づいたら、追加のリマインドをLINE Push通知で送信。

**想定データモデル追加**
- 代行運転・タクシー電話番号の手動登録テーブル（地域×業種×電話番号×表示名）。
- 飲酒セッション状態（開始時刻・杯数・予測時刻・GPS座標）を保持する新モデル。既存`CheckupReminder`とは別建てにする。

**未確定・今後詰める点**
- 手動登録の対象エリア（優先エリアの洗い出し）
- Google Places APIフォールバック時のAPI費用・利用規約上の扱い
- GPS座標・飲酒セッションデータの保持期間とプライバシー配慮（`app/privacy`への反映要否）
...
- **P1（LINE補助チャネルの充実）**: LINEのみで記録・利用しているユーザーへのリマインド対応（未記入リマインド・毎朝メッセージ等、既知の未対応項目への対応）。加えて、飲酒安全サポート（3.13節、飲み始めタイマー＋代行/タクシー番号のワンタップ発信）を新規追加。
